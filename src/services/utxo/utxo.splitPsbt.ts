...
export const redeemUtxoSplitPsbt = (
  wallet: any,
...
export const utxoSplitPsbt = (
  ...
  let outputSize = Math.floor(
    (inputUtxoSumValue - redeemFee) / SEND_UTXO_FEE_LIMIT
  );
  inputUtxoArray.forEach((utxo) => {
    psbt.addInput({
      hash: utxo.txid,
  ...
  psbt.addOutput({
    address: wallet.address,
    value: Math.floor(
      inputUtxoSumValue - redeemFee - outputSize * SEND_UTXO_FEE_LIMIT
    ),
  });
