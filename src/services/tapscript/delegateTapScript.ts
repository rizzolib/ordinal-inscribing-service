...
export const delegateTapScript = async (
  inscriptionData: IDelegateInscription
) => {
  let tapScript: Array<any> = [toXOnly(keyPair.publicKey), opcodes.OP_CHECKSIG];
  ...
  const inscriptionTransactionBuffer = Buffer.from(
    parentInscriptionTransactionID,
    "hex"
  ).reverse();
  ...
  if (parseInt(index, 10) != 0) {
    const indexBuffer = Buffer.from(
      parseInt(index, 10).toString(16).padStart(2, "0"),
      "hex"
    ).reverse();
  ...
  const DelegateinscriptionTransactionBuffer = Buffer.from(
    delegateInscriptionTransactionID,
    "hex"
  ).reverse();
  ...
  if (parseInt(DelegateIndex, 10) != 0) {
    const DelegateIndexBuffer = Buffer.from(
      parseInt(DelegateIndex, 10).toString(16).padStart(2, "0"),
      "hex"
    ).reverse();
  ...
  for (let i = 0; i < inscriptionData.delegateIds.length; i++) {
    ...
    subScript.push(opcodes.OP_FALSE, opcodes.OP_IF, Buffer.from("ord", "utf8"));
    ...
    subScript.push(opcodes.OP_ENDIF);
