...
const fileTapScript = (inscriptionData) => __awaiter(void 0, void 0, void 0, function* () {
    let tapScript = [
        (0, buffer_2.toXOnly)(keyPair.publicKey),
        bitcoinjs_lib_1.opcodes.OP_CHECKSIG
    ];
    let pointers = [];
    inscriptionData.files.forEach((item, index) => {
    ...
    for (let i = 0; i < inscriptionData.files.length; i++) {
        ...
        let subScript = [];
        subScript.push(bitcoinjs_lib_1.opcodes.OP_FALSE, bitcoinjs_lib_1.opcodes.OP_IF, Buffer.from("ord", "utf8"), 1, 1, Buffer.from(inscriptionData.files[i].mimetype, "utf8"));
...
exports.fileTapScript = fileTapScript;
